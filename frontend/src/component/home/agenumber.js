...
export default function Age({location,startDate,endDate}) {
    ...
    useEffect(() => {
        if (defData?.age?.data) {
            setBarData({
                labels: defData?.age?.data && defData?.age?.data?.length > 0 ? defData?.age?.data[1]?.map((item) => { return item.Host }) : [],
                datasets: [{
                    label: "Host",
                    data: defData?.age?.data && defData?.age?.data?.length > 0 ? defData?.age?.data[1]?.map((item) => { return item.Count }) : [],
                    backgroundColor: ['#33b2df', '#546E7A', '#d4526e', '#13d8aa', '#A5978B', '#2b908f', '#f9a3a4', '#90ee7e',
                '#f48024', '#69d2e7'
              ],
                    borderColor: ['#33b2df', '#546E7A', '#d4526e', '#13d8aa', '#A5978B', '#2b908f', '#f9a3a4', '#90ee7e',
    ...
    useEffect(() => {
        dispatch(getDefaultData(` [dbo].[GetHostDistribution]
    ...
    return (
        <div>
            <BarGraph defData={defData?.age?.data} barData={barData} />
