...
export default function GenderCombineVsUnknown({location,startDate,endDate,isChange}) {
    ...
    useEffect(() => {
        if (defData?.genderData?.data) {
            setBarData({
                labels: defData?.genderData?.data && defData?.genderData?.data?.length > 0 ? defData?.genderData?.data[1]?.filter((item) => item.Gender != "unknown")?.map((item) => { return item.Gender }) : [],
                datasets: [{
                    label: 'Gender Data',
                    data: defData?.genderData?.data && defData?.genderData?.data?.length > 0 ? defData?.genderData?.data[1]?.filter((item) => item.Gender != "unknown")?.map((item) => { return item.Count }) : [],
                    backgroundColor: ['#33b2df', '#546E7A', '#d4526e', '#13d8aa', '#A5978B', '#2b908f', '#f9a3a4', '#90ee7e',
                '#f48024', '#69d2e7'
              ],
                    borderColor: ['#33b2df', '#546E7A', '#d4526e', '#13d8aa', '#A5978B', '#2b908f', '#f9a3a4', '#90ee7e',
    ...
    useEffect(() => {
        dispatch(getDefaultData(` [dbo].[GetGenderDistribution]
    ...
    return (
        <div>
            ...
            <BarGraph loading={defData?.genderData?.loading} defData={defData?.genderData?.data} barData={barData} />
